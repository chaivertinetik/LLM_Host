...
graph_requirement = [   
                        ...
                        'When loading data from internal Google Cloud SQL-backed GeoJSON endpoints such as /cloudsql/geojson, preserve the COMPLETE URL exactly as provided in the data locations. These URLs are already ROI-aware and should be treated like ready-to-read GeoJSON sources with no ArcGIS token handling or pagination added by you.'
...
operation_requirement = [
    ...
    "If the query asks for something like show me the tallest ash tree, that means they want a visual result so the final output should be a geodataframe that will be sent to the map on ArcGIS, dont generate a textual summary in this case, but if the user asks for what the height of the tallest tree is then the result should be a textual summary containing that value (string).",
    "For requests like 'show ash trees', 'display oak trees', or 'highlight healthy trees', return the filtered GeoDataFrame itself as the final result so it can be pushed to the ArcGIS output layer; do not collapse it into IDs or a plain text summary.",
    ...
    "When filtering species like 'Ash', apply attribute filters BEFORE spatial operations to reduce compute. Prefer a case-insensitive contains match that also captures Fraxinus values before doing any spatial work.",
...
assembly_requirement = ['You can think step by step. ',
                    ...
                    "Prefer datasets[...] GeoDataFrames and dataset_catalog metadata whenever available; only read URLs when no preloaded dataset exists.",
                    "For count-only questions, prefer dataset_catalog feature_count or a count endpoint instead of loading full geometry pages.",
...
direct_request_requirement = [
                        ...
                        "If you need to make a map and the map size is not given, set the map size to 15*10 inches.",
                        "Prefer datasets[...] GeoDataFrames and dataset_catalog feature_count metadata whenever available; avoid downloading a URL again if a dataset key exists.",
                        "For count-only questions, use feature_count from dataset_catalog or a count endpoint instead of loading full geometries.",
