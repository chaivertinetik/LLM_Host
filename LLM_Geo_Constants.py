...
operation_requirement = [                         
                        ...
                        "Before performing any distance-based spatial operations, reproject geometries to a projected CRS with metric units (e.g., EPSG:27700 or the appropriate UTM zone), if they are not already.To find features within a specified distance from a target feature, compute pairwise distances using:gdf['distance_to_target'] = gdf.geometry.distance(target_geom).Then filter using: within_range = gdf[(gdf['distance_to_target'] <= max_distance) & (gdf.geometry != target_geom)] Replace max_distance with the desired threshold (e.g., 30). Avoid using geographic (lat/lon) coordinates or geodesic methods unless specifically required.",
