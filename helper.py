...
def extract_content_from_LLM_reply(response):
    ...
    if stream:       
        ...
        # Check if the response has a 'text' attribute (Vertex AI)
        ...
        # Raise an error if the structure is unrecognized
        # raise AttributeError("Unrecognized response structure: 'text' or 'choices' attribute not found.")
    return response
...
from vertexai.generative_models import GenerativeModel  # Import the GenerativeModel
...
def get_LLM_reply(prompt="Provide Python code to read a CSV file from this URL and store the content in a variable.",
                  ...
                  retry_cnt=3,
                  sleep_sec=10):
    # Initialize the Vertex AI model using the model_name
    ...
    while not isSucceed and count < retry_cnt:
        try:
            count += 1
            # Correct method based on what you find using dir(), e.g., `generate()`
            response = model.generate_content( "Provide Python code to read a CSV file from this URL and store the content in a variable." # or model.predict() depending on the method
                # Assuming 'prompt' is the correct argument for generating content
                # temperature=temperature,
                # stream=stream
            )
            isSucceed = True  # Exit the loop if the response is successful
    ...
    if isSucceed:
        # Since the response is a string (not iterable), directly assign it to content
        if stream:
            # If the response is in streaming mode, concatenate the chunks
            content =  response.text
            # ''.join(chunk['content'] for chunk in response)
        else:
            # If the response is a full string, assign it directly to content
            content = response.text  # Assuming response has a 'text' attribute for a string

        print("Got LLM reply:", content)
        return content
    else:
